    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            var factory = new ConnectionFactory();
            {
                ...
                factory.Port = 5672;
                factory.VirtualHost = "TestVirtual"; //选择虚拟主机
                factory.UserName = "sa";
                factory.Password = "123456";
                factory.AutomaticRecoveryEnabled = true; //开启自动恢复连接(默认为false，在异常中断时无法自动恢复连接)
            ...
            #region 创建一个普通消息消费者
            {
                ...
                consumer.Received += (sender, e) =>
                {
                    ...
                    #region 消息处理失败时
                    {
                        ...
                        System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t(1.2)处理失败：\r\n\t[deliveryTag={_message.DeliveryTag}]\r\n\t[retryCount={retryCount}]");
            ...
            #region 创建一个失败消息消费者
            {
                ...
                consumer.Received += (sender, e) =>
                {
                    ...
                    System.Console.WriteLine();
                    System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t(2.0)发现失败消息：\r\n\t[deliveryTag={_message.DeliveryTag}]\r\n\t[consumerID={_message.ConsumerTag}]\r\n\t[exchange={_message.Exchange}]\r\n\t[routingKey={_message.RoutingKey}]\r\n\t[content={_content}]");
                };
            ...
            System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t正在运行中...");

            var cmd = default(string);
            while ((cmd = System.Console.ReadLine()) != "close")
            #region 模拟正常消息发布
            {
                ...
                System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t发送成功：{msgContent}");
                System.Console.WriteLine();
            }
            #endregion

            System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t正在关闭...");
            ...
            System.Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss.fff")}\t运行结束。");
